 ...
 */
type ComponentRegistry = Record<string, ReturnType<typeof defineAsyncComponent>>;
 ...
 */
export function getWidget(name: string): ReturnType<typeof defineAsyncComponent> | undefined {
...
export function resolveWidget(blockConfig: BlockConfig): Component | string {
  ...
  console.log(`Resolving widget "${widgetName}" for block "${blockConfig.name}"`);

  if (!widget) {
    console.warn(`[ComponentResolver] Predefined widget "${widgetName}" not found in registry`);
    return 'div';
  }
 ...
 * @returns Component or undefined if not found
 */
export function getFieldComponent(name: string): ReturnType<typeof defineAsyncComponent> | undefined {
 ...
 */
export function resolveFieldComponent(componentName: string, silent = false): Component | string {
  const component = fieldRegistry[componentName];

  if (!component) {
    if (!silent) {
      console.info(`[ComponentResolver] Field component "${componentName}" not found in registry`);
    }
    return 'div';
  }
